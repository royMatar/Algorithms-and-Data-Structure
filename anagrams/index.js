...
function anagrams(stringA, stringB) {
  //Sanitize the input string (removing spaces and non alphanumeric characters)
...
// //Solution 2: (Bad option because the time complexity is O(N*LogN))
// //Words that are anagrams of each other , will be the same word if their letters are sorted Alphabetically!
// function anagrams(stringA, stringB) {
//   stringA = stringA.toLowerCase().replace(/[\W_]+/g, "");
//   stringB = stringB.toLowerCase().replace(/[\W_]+/g, "");

//   //Turn strings to array of characters, use .sort(), turn sorted arrat back intro string
//   //check if both of these newly sorted strings are equal to each other
