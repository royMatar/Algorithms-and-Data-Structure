...
function capitalize(str) {
  const arrOfWords = str.split(" "); //Space between "" to split words not letters
  const arrOfWordsCapitalized = [];

  for(let i=0; i<arrOfWords.length;i++){
    const word = arrOfWords[i];
    arrOfWordsCapitalized.push(word[0].toUpperCase() + word.slice(1).toLowerCase()); //slice(1) removes the first letter from the word
  }
  return arrOfWordsCapitalized.join(" "); //Space between "" to split words not letters
}
