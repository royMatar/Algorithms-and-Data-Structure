...
function caesarCipher(str, shift) {
  // Define an array of letters in the alphabet
  const alphabetArr = ["abcdefghijklmnopqrstuvwxyz"].split("");
  // Initialize an empty string to hold the result
  let res = "";

  // Loop through each character in the input strin
  for (let i = 0; i < str.length; i++) {
    // Get the current character and its index in the alphabet array
    const char = str[i];
    const idx = alphabetArr.indexOf(char);
    // If the character is not in the alphabet (e.g. a space or punctuation), add it to the result as is
    if (idx === -1) {
      res += char;
      continue;
    }
    // Calculate the new index of the character after shifting by the specified amount
